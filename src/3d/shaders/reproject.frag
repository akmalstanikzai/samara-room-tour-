...
#define PLANE_DISTANCE (20.0)
#define WORLD_DISTANCE (10.0)
#define NORMAL_DISTANCE (1.0)
...
float worldDistanceDisocclusionCheck(
  const vec3 worldPos,
  const vec3 lastWorldPos,
  const float distFactor
) {
...
float normalDisocclusionCheck(
  const vec3 worldNormal,
  const vec3 lastWorldNormal,
  const float distFactor
) {
  // Calculate the distance between the current and last world position
  return min(1.0 - dot(worldNormal, lastWorldNormal), 1.0) /
  NORMAL_DISTANCE *
  distFactor;
}

float validateReprojectedUV(
  const vec2 reprojectedUv,
  const vec3 worldPos,
  const vec3 worldNormal,
  const bool isHitPoint
) {
  if (
    reprojectedUv.x > 1.0 ||
    reprojectedUv.x < 0.0 ||
  ...
  getVelocityNormalDepth(
    dilatedReprojectedUv,
    lastVelocity,
    lastWorldNormal,
    lastDepth
  );
  ...
  float distFactor = 1.0 + 1.0 / (viewZ + 1.0);

  float disoccl = 0.0;

  disoccl += worldDistanceDisocclusionCheck(worldPos, lastWorldPos, distFactor);
  disoccl += planeDistanceDisocclusionCheck(
    worldPos,
    lastWorldPos,
    worldNormal,
    distFactor
  ...
  float confidence = 1.0 - min(disoccl, 1.0);
...
vec2 reprojectHitPoint(const vec3 rayOrig, const float rayLength) {
  ...
  float m = min(max(0.0, roughness - 0.25) / 0.25, 1.0);

  return reprojectedHitPoint.xy;
}

vec3 getReprojectedUV(
  const bool doReprojectSpecular,
  const float depth,
  const vec3 worldPos,
  const vec3 worldNormal
) {
  if (doReprojectSpecular) {
    ...
    float confidence = validateReprojectedUV(
      reprojectedUv,
      worldPos,
      worldNormal,
      true
...
vec4 BiCubicCatmullRom5Tap(sampler2D tex, vec2 P) {
  ...
  vec4 Ct =
    textureLod(tex, vec2(Sample[1].x, Sample[0].y), 0.0) * sampleWeight[0];
  vec4 Cl =
    textureLod(tex, vec2(Sample[0].x, Sample[1].y), 0.0) * sampleWeight[1];
  vec4 Cc =
    textureLod(tex, vec2(Sample[1].x, Sample[1].y), 0.0) * sampleWeight[2];
  vec4 Cr =
    textureLod(tex, vec2(Sample[2].x, Sample[1].y), 0.0) * sampleWeight[3];
  vec4 Cb =
    textureLod(tex, vec2(Sample[1].x, Sample[2].y), 0.0) * sampleWeight[4];
